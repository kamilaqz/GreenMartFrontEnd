...
const CreateProduct = () => {
  ...
  setLoginDisabledTimeout(setTimeout(() => setIsLoginDisabled(false), 8000))
    ...
    try{
      const response = await httpService.createProduct(data)
    ...
    <>
      ...
      <AppBar position="relative">
        <Toolbar>
          ...
          <Typography variant="h6" color="inherit" noWrap style={{ margin: 'auto' }} className="custom-font">
            GreenMart
          </Typography>
        </Toolbar>
      </AppBar>
    ...
    <Container component="main">
        <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" sx={{ mt: 4 }}>
          ...
          <Box sx={{mt: 2}} component= "form" onSubmit={handleRegisterProduct} display="flex" flexDirection="column" alignItems="center" justifyContent="center">
            <TextField required fullWidth margin="normal" name='name' type="name" label="Product Name"/>
            <TextField required fullWidth margin="normal" name="category" type="category" label="Product Category"/>
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                <TextField required fullWidth margin="normal" name="id" type= "id" label="Product Code"  inputProps={{ pattern: "[0-9]{1,3}", title: "Product Code deve ser um número de 3 dígitos"}}/>
              </Grid>
            </Grid>
